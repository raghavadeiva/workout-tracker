...
**100% Client-Side Recommendation Engine — Pure Cosine Similarity over a Generated 873-Exercise Library**

- **No external API calls** — all vector math and similarity ranking computed in-browser
- **No remote model loading** — vectors generated at build time by `scripts/exercises-seeder.js` from `exercises.json` into `exerciseVectors.generated.ts`
- **Pure Cosine Similarity** — the sole similarity metric for exercise substitution ranking
- Formula: `cosineSimilarity(a, b) = dot(a, b) / (||a|| * ||b||)` — all math loops to `vector.length`, so dimensionality is data-driven (currently 20)
- Result: angle-based similarity in [-1, 1]; activation vectors are non-negative so practical range is [0, 1]

### Vector Embedding Schema (Phase 14 — 20 dimensions, all muscle groups)

Generated by `scripts/exercises-seeder.js`; regenerate with `node scripts/exercises-seeder.js`.

| Slots | Content | Encoding |
|-------|---------|----------|
| 0–16 | abdominals, abductors, adductors, biceps, calves, chest, forearms, glutes, hamstrings, lats, lower back, middle back, neck, quadriceps, shoulders, traps, triceps | `primaryMuscles` → 1.0 · `secondaryMuscles` → 0.5 (primary wins overlap) |
| 17–19 | spare slots (zeroed) | reserved for future muscle groups |

Dimension labels live in `GENERATED_MUSCLE_DIMENSIONS` (index-aligned with every vector).

> **Legacy schema (10 muscle + 4 equipment + 6 movement dims) is retired.** Equipment is no longer encoded in vectors — it is a record field with bucket normalization (see below). Do not reintroduce dimension-based equipment filtering.

### Equipment Bucket Filtering (replaces vector-dimension filtering)

Raw dataset equipment strings map to four UI filter buckets via `normalizeEquipment()`:
`barbell`→barbell · `dumbbell`,`kettlebells`→dumbbell · `machine`,`cable`→machine · `body only`→bodyweight.
Unmapped strings (`other`, bands, medicine ball, foam roll, exercise ball, empty) → **no bucket = always available**, so unknown equipment never hides a substitute.

### Recommendation Flow
1. User opens the swap dropdown for an exercise (`ExerciseSwap`)
2. Target vector looked up by display name in `EXERCISE_LIBRARY`
3. Cosine similarity computed against all other library records (~873 × 20 mults — sub-millisecond)
4. Results filtered by equipment buckets when the user has preferences set
5. Top-N ranked by score returned to UI

### Data Storage
- Vectors live in code (`exerciseVectors.generated.ts`) — the source of truth for the engine
- The IndexedDB `recommendations` store (schema v4) holds seeded copies for future offline query features; **seeding is version-gated** (`v2-873` marker record) and promise-coalesced so the 873-record write happens once per dataset change, not per app load
- The engine reads from the in-memory library, never from IndexedDB — stale/missing DB rows are harmless

### Performance
- Similarity computation memoized per target exercise + equipment filter in `useRecommendations`
- No blocking of the main thread (pure JS math, ~35k multiply-accumulates worst case)

### Files
```
scripts/exercises-seeder.js       # exercises.json → exerciseVectors.generated.ts
src/features/recommendations/
  vectorUtils.ts                  # dot product, magnitude, cosine similarity (dimension-agnostic)
  exerciseVectors.generated.ts    # AUTO-GENERATED: 873 records × 20-dim vectors — do not hand-edit
  exerciseVectors.ts              # App-facing API over the library:
                                  #   EXERCISE_LIBRARY, getRecommendations(),
                                  #   getAllExerciseNames(), normalizeEquipment(),
                                  #   getEquipmentBuckets(), getLibraryEntry()
  useRecommendations.ts           # reactive hook (memoized + equipment bucket filtering)
  components/
    ExerciseSwap.tsx              # swap icon button + spring dropdown with alternatives
                                  # (rendered in each exercise card header; computes only
                                  # while open; equipment filter from useWorkoutSession)
```
...
> The IndexedDB `recommendations` store keeps seeded copies (`saveExerciseEmbedding` /
> `getExerciseEmbedding` / `getAllExerciseEmbeddings` in `database.ts`) for future offline
> queries; the live engine reads from the in-memory library.
...
| 9 Recommendations | 873-exercise generated library, cosine similarity client-side, equipment bucket filtering, in-workout swap ✓ |
